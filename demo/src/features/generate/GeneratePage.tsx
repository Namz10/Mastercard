...
import { NowHappening } from "@/explain/NowHappening";
import { PipelineDiagram } from "@/explain/PipelineDiagram";
import { LiveCounters } from "@/components/ui/LiveCounters";
import { FamilyInjectBars } from "@/components/ui/FamilyInjectBars";
    ...
    >
      ...
      {running ? (
        <div className="flex-1 min-h-[420px] flex flex-col gap-2">
          <NowHappening />
          <PipelineDiagram mode="generate" activeVerb={narration?.verb} activeBody={narration?.happening} />
          <LiveCounters
            customers={progress.customersDone ?? (narration?.artifacts?.customers as number | undefined)}
            events={progress.eventCount ?? (narration?.artifacts?.events as number | undefined)}
          />
          {narration?.visual === "families" && narration.artifacts?.family ? (
            <FamilyInjectBars
              counts={{ [String(narration.artifacts.family)]: Number(narration.artifacts.count ?? 0) }}
              className="shrink-0"
            />
          ) : null}
          <div
