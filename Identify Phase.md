...
**Relationship to the main brief.** This document assumes the lifecycle taxonomy, typology catalog, and scoring criteria in `research_brief.md` (Section 3) as the *content* map. This document is the *process* by which we populate, validate, and confidence-score that catalog.
...
**Confirmation rule:** a vector is marked **"confirmed"** only if it clears Tier 1 or Tier 2 alone, **or** is corroborated by two independent sources at Tier 3 or better. Anything resting on a single Tier 3–5 source (e.g. one vendor's "1,210% surge" statistic) is tagged **"reported, unverified"** in the catalog and carried forward with that label intact — not silently upgraded.
...
|---|---|---|
| **Technical / network-footprint vectors** (bot-driven onboarding, credential stuffing, card testing, scanning) | Cross-check against live public telemetry (GreyNoise, Shadowserver, DShield) | These leave an observable network trace; live data confirms the underlying infrastr Documented incident case studies, regulator alerts, published red-team/liveness-bypass research | These attacks happen through human interaction, not network intrusion — traditional honeypots and sensors cannot observe them, so documentary evidence from Tier 1–2 sources is the only real corroboration available |
...
| `control_bypassed` | e.g. liveness check, voice biometric, OTP, human callback, velocity rule |
...
| Standalone automated web-scraping infrastructure as a demo feature | Adds engineering risk for a stage judges spend little time on, unless the live-ingestion "researcher agent" itself is shown as a demo feature (optional stretch goal per Section 6.2 of the main brief) |ucture is active *now*, not just historically reported |
| **Human / social-engineering vectors** (deepfake KYC, voice clone, APP scam, BEC impersonation) |
...
2. **Canary mode** — pins to a single `canary_eligible` entry to reproduce one documented case exactly, for validation against the current blue model.
