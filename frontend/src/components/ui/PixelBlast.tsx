...
function PixelBlastFallback({
  ...
  return (
    <div
      className={cn("pixel-blast-fallback absolute inset-0", className)}
      style={{ ...style, ["--pixel-color" as string]: color }}
      aria-hidden
    />
  );
}

/** React Bits PixelBlast shaders — liquid/ripples off on booth path. */
export function PixelBlast({
  ...
  if (reduced) {
    return <PixelBlastFallback color={color} className={className} style={style} />;
  }
