...
const VERB_META: Record<string, { icon: LucideIcon; label: string }> = {
  COLLECT: { icon: Globe2, label: "Collect" },
  EXTRACT: { icon: FileSearch, label: "Extract" },
  RANK: { icon: Layers3, label: "Rank" },
  GROUND: { icon: Link2, label: "Ground" },
  PROPOSE: { icon: Sparkles, label: "Propose" },
  REPLAY: { icon: BookMarked, label: "Replay" },
};

function ThreadIcon({ verb }: { verb: string }) {
  const meta = VERB_META[verb.toUpperCase()] ?? VERB_META.COLLECT;
...
export function CatalogThread({
  lines,
  running = false,
  emptyLabel = "Starting catalog research…",
}: {
  lines: OpsTapeLine[];
  running?: boolean;
  emptyLabel?: string;
  ...
  return (
    <div className="catalog-thread panel flex flex-col h-full min-h-0" data-demo="catalog-thread">
      <div className="h-9 px-3 border-b border-border flex items-center justify-between shrink-0">
        <span className="font-mono text-[11px] uppercase text-ink-faint">Catalog thread</span>
