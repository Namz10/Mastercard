...
import { useDecisioningState } from "./useDecisioningState";

export function useDecisioning() {
  const qc = useQueryClient();
  const { runId, setLastScore } = useLatestRun();
  const { training, setTraining, score, setScore } = useDecisioningState();
  ...
  const fit = useMutation({
    ...
    onSuccess: () => {
      setTraining({ status: "completed", completedAt: new Date().toISOString() });
  ...
  return { fit, score: scoreMutation, runId, scoreData: score, training };
}
